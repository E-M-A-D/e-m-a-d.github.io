...
var emissions_scale_rainbow = d3.scale.linear()
...
var emissions_scale_gradient = d3.scale.linear()
                              .domain([1000,      100000,  300000,      500000,    1000000,  2000000,  3000000,    6000000, 12000000])
                             .range(["#ffffe5", "#fff7bc", "#fee391",  "#fec44f", "#fe9929", "#ec7014", "#cc4c02", "#993404", "#662506"]);

var emissions_per_capita_scale_rainbow = d3.scale.linear()
                              .domain([0.1,       1,        2,             3,    5,   10,  15, 20, 44])
                             .range(["darkgreen", "green", "yellowgreen",  "yellow", "orange", "darkorange", "orangered", "red", "darkred"]);

var emissions_per_capita_scale_gradient = d3.scale.linear()
                              .domain([0.1,       1,        2,             3,    5,  10,  15, 20, 44])
                             .range(["#ffffe5", "#fff7bc", "#fee391",  "#fec44f", "#fe9929", "#ec7014", "#cc4c02", "#993404", "#662506"]);

var emissions_productivity_scale_rainbow = d3.scale.linear()
                              .domain([1000,      100000,  300000,         500000,    1000000,  2000000,  3000000, 6000000, 12000000])
                              .range(["darkred", "red", "orangered",  "darkorange", "orange", "yellow", "yellowgreen", "green", "darkgreen"]);

var emissions_productivity_scale_gradient = d3.scale.linear()
                              .domain([1000,      100000,  300000,      500000,    1000000,  2000000,  3000000,    6000000, 12000000])
                              .range(["#662506", "#993404", "#cc4c02",  "#ec7014", "#fe9929", "#fec44f", "#fee391", "#fff7bc", "#ffffe5"]);
...
d3.json("js/maps/modified_map.json",
        ...
        map_svg.append("g")
               ...
               .attr('id', 'map_legend_axis')
               .call(gradient_axis)
               .append("text")
               .attr('x', 305)
               .attr('y', 2)
               .attr('fill', 'black')
               .attr('id', 'map_legend_text')
               .text("Tons of CO2");
...
//*****************************************************************************************
//*****************************************************************************************
//Map Coulouring by radio selection
...
d3.csv("data/2011_data.csv", function(data)
{
    ...
    draw_map_by_metric_and_scale(country_emissions_data, 'total_co2', 'rainbow')
    d3.selectAll("#map_radio_option").on("click",
                    function()
                    {
                        radio_metric_selection = document.querySelector('input[name="metric_radio"]:checked').value;
                        radio_colour_selection = document.querySelector('input[name="map_colour_radio"]:checked').value;
...
var numeric_params = ["co2_per_capita", "total_emissions", 'gdp'];//, "emissions_productivity"];
...
hidden_divs = d3.selectAll(".hidden_at_start");
hidden_divs.style("visibility", "visible");
hidden_divs.style('opacity', 1)


//*****************************************************************************************
//*****************************************************************************************
//Map Coulouring by year, this part is mostly disabled now. But the helper functions are still used,
//this is why the code is still kept.
...
function init_cp_meters(svg_elemet)
{
    ...
    //We have two types of scales, one where the lower value is a good thing (Like amount of emissions)
            ...
            .attr("y", function (d)
                       {
                        ...
                        //an array and the data is contained in the first element, this is
